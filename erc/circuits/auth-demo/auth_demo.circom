...
template AuthDemo() {
    ...
    // ===== Private witnesses (signed intent fields per spec §15.6) =====
    ...
    // ===== Step 3: compute the two-level transactionIntentDigest (spec §15.6),
    // enforce equality =====
    component fieldsHash = TransactIntentFieldsHash();
    fieldsHash.poolAddress                           <== poolAddress;
    fieldsHash.authVerifier                          <== authVerifier;
    fieldsHash.authorizingAddress                    <== authorizingAddress;
    fieldsHash.operationKind                         <== operationKind;
    fieldsHash.tokenAddress                          <== tokenAddress;
    fieldsHash.recipientOwnerNullifierKeyHash        <== recipientOwnerNullifierKeyHash;
    fieldsHash.amount                                <== amount;
    fieldsHash.feeNoteRecipientOwnerNullifierKeyHash <== feeNoteRecipientOwnerNullifierKeyHash;
    fieldsHash.feeAmount                             <== feeAmount;
    fieldsHash.publicRecipientAddress                <== publicRecipientAddress;
    fieldsHash.authorizedSubmitter                   <== authorizedSubmitter;
    fieldsHash.downstreamActionCommitment            <== downstreamActionCommitment;
    fieldsHash.executionConstraintsFlags             <== executionConstraintsFlags;
    fieldsHash.lockedOutputBinding0                  <== lockedOutputBinding0;
    fieldsHash.lockedOutputBinding1                  <== lockedOutputBinding1;
    fieldsHash.lockedOutputBinding2                  <== lockedOutputBinding2;
    fieldsHash.nonce                                 <== nonce;
    fieldsHash.validUntilSeconds                     <== validUntilSeconds;
    fieldsHash.executionChainId                      <== executionChainId;

    component intentHash = TransactionIntentDigest();
    intentHash.transactIntentFieldsHash <== fieldsHash.out;
    intentHash.policyDataHash           <== policyDataHash;
