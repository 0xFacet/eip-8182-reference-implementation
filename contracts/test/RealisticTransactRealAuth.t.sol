...
/// @notice End-to-end integration test for the realistic-auth flow:
///         pool proof (Groth16/BN254, mock precompile) + auth proof
///         (UltraHonk via bb, deployed Solidity verifier). Mirrors
///         RealisticTransact.t.sol's structure but with RealAuthVerifier
///         instead of DemoAuthVerifier. Reads
///         build/integration_real/session.json (produced by
///         scripts/integration/build_real_session.js).
contract RealisticTransactRealAuthTest is Test, InstallSystemContractsBase {
    using stdJson for string;

    address internal constant PROOF_VERIFY_PRECOMPILE_ADDRESS =
        0x0000000000000000000000000000000000000030;

    // --- Pinned witness identities (must match scripts/noir/gen_real_pool_witness_input.js) ---
    ...
    function setUp() public {
        ...
        session = vm.readFile("build/integration_real/session.json");
    ...
    function testRealisticRealAuthTransactSucceeds() public {
