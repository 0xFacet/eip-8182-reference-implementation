...
// Derived from zemse/poseidon2-evm LibPoseidon2Yul.poseidon2_core; the only
// change is the final assignment (all four state elements are returned instead
// of just state0), which is required for sponge chaining across absorbs of more
// than `rate` = 3 input elements. Round constants and MDS matrices unchanged.

pragma solidity >=0.6.0;

/// @notice Full-state Poseidon2 permutation — companion to LibPoseidon2Yul.poseidon2_core.
/// @dev Exposes (r0, r1, r2, r3) so sponge wrappers can chain permutations.
library LibPoseidon2Permute {
    function permute4(uint256 s0, uint256 s1, uint256 s2, uint256 s3)
        internal
        pure
